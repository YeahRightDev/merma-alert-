...
export async function runMigrations() {
  ...
  try {
    await client.query(`
      ...
      -- Agregar columnas nuevas a productos si ya existe la tabla (migración segura)
      DO $$ BEGIN
        ...
        ALTER TABLE productos ADD COLUMN IF NOT EXISTS costo_compra DECIMAL(10,2) DEFAULT 0;
      ...
      SELECT
        l.id, l.negocio_id,
        p.nombre AS producto, p.marca, p.sku, p.unidad, p.imagen_url,
        p.precio_venta, p.costo_compra,
        l.cantidad, l.cantidad_inicial, l.fecha_caducidad, l.notas,
